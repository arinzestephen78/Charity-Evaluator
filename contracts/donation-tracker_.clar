...
(define-public (update-transparency-score (charity-id uint) (new-score uint))
  (let
    ...
    (ok (map-set charities charity-id (merge charity { transparency-score: new-score })))
  )
)

(define-public (update-total-donations (charity-id uint) (amount uint))
  (let
    (
      (charity (unwrap! (map-get? charities charity-id) (err u404)))
    )
    (ok (map-set charities charity-id (merge charity { total-donations: (+ (get total-donations charity) amount) })))
  )
)
